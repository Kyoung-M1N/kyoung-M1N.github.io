...
- 비관적 락 이용
  ...
  위와 같이 repository의 메서드에 비관적 락 설정을 추가한 뒤에 30명의 학생이 수강 인원이 20명인 강의에 대해 동시에 수강 신청을 하는 경우를 테스트한 결과, `select * from lecture where lecture_id = ? for update`쿼리가 실행되는 것을 확인할 수 있었다. 이로 인해 조회 시점에서 트랜잭션이 배타적 락을 획득함을 알 수 있고, 이후 트랜잭션이 커밋되기 전까지의 로직이 순차적으로 실행되며 요청 순서에 다른 로직의 실행을 보장하면서 동시성 문제가 해결되는 것을 확인할 수 있었다.
