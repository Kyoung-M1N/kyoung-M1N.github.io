...
낙관적 락은 배타적 락에 의한 트랜잭션 대기가 적게 발생하는 대신, 버전 불일치로 인한 트랜잭션 롤백 및 재시도에 의한 시간 지연이 존재하기 때문에 평균 응답 시간과 처리량이 높은 반면에 비관적 락은 충돌에 의한 롤백이 발생하지 않는 대신에 배타적 락에 의한 트랜잭션 대기가 빈번하게 발생하여 처리량이 낮고 평균 응답 속도가 빠른 것을 확인할 수 있었다.
