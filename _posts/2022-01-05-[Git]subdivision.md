...
위의 두 명령어는 `git branch`와는 달리 브랜치를 이동하는 명령어지만 각각 `-b`옵션과 `-c`옵션을 통해 세로운 브랜치를 생성함과 동시에 이동을 하는 명령어이다.
...
이 때 `git restore`은 `git checkout`과 달리 `--staged`옵션으로 stage에 올라간 파일까지 되돌릴 수 있다.
